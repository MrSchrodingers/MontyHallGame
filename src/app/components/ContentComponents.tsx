...
export const ContentComponent: React.FC = () => {
  return (
    <div className="relative w-full h-svh min-h-screen">
      <div className='absolute w-full h-full z-0'>
        <Canvas>
          <Stars />
        </Canvas>
      </div>
      <div className='grid lg:grid-cols-2 px-4 py-2 justify-between items-center mx-2 lg:gap-8 xl:gap-0 lg:py-16'>
        <motion.div 
        ...
        <motion.div 
          className="container flex items-center h-screen px-6 justify-center hover:cursor-pointer hover:transition-transform hover:transform hover:scale-105"
          initial={{ opacity: 0, x: 100 }}
          whileInView={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: 100 }}
          transition={{ duration: 4 }}
        >
          <Link href='/game' className='h-full w-full'>
            <Model3D />
          </Link>
        </motion.div>
      </div>   
